/**
 * FROZEN BASELINE — a verbatim copy of src/export/palletpdf.js exactly as it
 * stood on `main` before the layer-plan-inset task (git commit e2b63d7's
 * parent chain, before PR #31 was merged; this file predates that work
 * entirely). Used ONLY by test/layerplan.test.html's bit-identical pin,
 * which imports both this and the live module and confirms a spec with no
 * `layerPlan` key produces byte-for-byte the same PDF from both — the
 * closest honest analog to "extraction is safe" this task has, since (per
 * its own CLAUDE.md entry) there was no pre-existing layer-plan drawing in
 * this file to literally extract. Never import this from application code.
 *
